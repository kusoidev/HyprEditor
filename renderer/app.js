...
class HyprEditor {
  ...
  async Init() {
    ...
    if (SECTIONS.length) {
      this.ActivateSection(SECTIONS[0], SECTIONS[0].subsections[0]);
    }
  }
  ...
  async LoadConfig(filePath) {
    ...
    try {
      ...
      this.ShowNotification(`Parse error: ${err.message}`, 'error');
      return;
    }
    ...
    if (changed) {
      ...
      if (ok) {
        ...
        return await this.LoadConfig(filePath);
      }
    }
  ...
  async SaveConfig() {
    ...
    if (segments.length === 0) {
      ...
      this.state.fileSegments = [{
        filePath: this.state.configPath,
        startLine: 0,
        lineCount: this.state.rawLines.length,
      }];
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (!res.ok) {
        this.ShowNotification(`Save failed (${seg.filePath.split('/').pop()}): ${res.error}`, 'error');
        return;
      }
  ...
  IsRiskySetting(setting, nextValue) {
  ...
  ConfirmAction({ title = 'Are you sure?', message = 'Are you sure you want to do this?', confirmText = 'Confirm', cancelText = 'Cancel', danger = false }) {
  ...
  GetPreviewType(setting, sectionPath = []) {
  ...
  RenderPreviewCard(setting, value, sectionPath = []) {
        ...
        return `
          <div class="preview-card">
            <div class="preview-meta">
              <div class="preview-title">${this.EscapeHtml(label)}</div>
              <div class="preview-desc">${this.EscapeHtml(desc)}</div>
            </div>
            ...
            <div class="preview-value">Current: ${this.EscapeHtml(String(safeValue))}</div>
          </div>
        `;
          ...
          <div class="preview-card">
            ...
            <div class="mock-window-wrap">
              <div class="mock-window mock-border" style="
                ...
                border-color:${setting.type === 'color' ? this.EscapeHtml(String(value || '#a6adc8')) : '#a6adc8'};
              ">
                <div class="mock-titlebar"></div>
                <div class="mock-body"></div>
              </div>
        ...
        return `
          <div class="preview-card">
            ...
            <div class="color-preview">
              <div class="color-swatch" style="background:${this.EscapeHtml(String(value || '#89b4fa'))}"></div>
            </div>
            <div class="preview-value">Current: ${this.EscapeHtml(String(safeValue))}</div>
          </div>
        `;
  ...
  UpdatePreview(setting, value, sectionPath = []) {
  ...
  BuildSidebar() {
    ...
    sidebar.addEventListener('click', e => {
      ...
      if (sec) {
        this.ActivateSection(sec, sec.subsections[0]);
      }
    });
  }
...
╚═╝  ╚═╝   ╚═╝   ╚═╝     ╚═╝  ╚═╝╚══════╝╚═════╝ ╚═╝   ╚═╝    ╚═════╝ ╚═╝  ╚═╝
  ...
  RenderActiveSection() {
    ...
    if (sec.id === 'about') {
      main.innerHTML = this.RenderAboutSection();
      return;
    }

    let html = `<div class="section-header">
      <h2 class="section-title">${sec.label}</h2>
      ${this.state.configPath ? `<span class="config-file-badge">${this.state.configPath.split('/').slice(-2).join('/')}</span>` : ''}
    </div>`;
    ...
    main.querySelectorAll('.sub-tab').forEach(tab => {
      tab.addEventListener('click', () => {
        ...
        if (found) {
          this.state.activeSubsection = found;
          this.RenderActiveSection();
        }
      });
    });
  ...
  GetValueFromPath(sectionPath, key) {
  ...
  GetListFromPath(sectionPath, key) {
  ...
  RenderControl(setting, value, sectionPath, lineIdx) {
      ...
      controlHtml = `
        <div class="autoset-row">
          ...
          <button class="btn-autoset"
            ...
            data-path='${pathStr}'
            data-default="${this.EscapeHtml(String(defaultVal))}"
      ...
      switch (setting.type) {
        ...
        default:
          controlHtml = `<input type="text" id="${id}" class="text-input"
            value="${this.EscapeHtml(value)}" placeholder="${!hasValue ? 'not set' : ''}"
    ...
    return `
      <div
        ...
        data-key="${setting.key}"
        data-label="${this.EscapeHtml(setting.label)}"
        data-desc="${this.EscapeHtml(setting.desc || '')}"
        data-type="${this.EscapeHtml(setting.type)}"
        data-section-path="${this.EscapeHtml(JSON.stringify(sectionPath))}"
      >
  ...
  RenderListSection(listDef, entries, sectionPath) {
    const pathStr = JSON.stringify(sectionPath);
    let html = `
      ...
      entries.forEach((entry, idx) => {
        html += `
          <div class="list-entry" data-index="${idx}">
            ...
            <input type="text" class="list-entry-input" value="${this.EscapeHtml(entry.value)}"
  ...
  AttachControlListeners(container, sub) {
    ...
    container.querySelectorAll('.text-input').forEach(input => {
      ...
      input.addEventListener('input', () => {
        clearTimeout(timer);
        timer = setTimeout(() => this.OnControlChange(input), 600);
      });
    });
    ...
    container.querySelectorAll('.list-entry-input').forEach(input => {
      ...
      input.addEventListener('input', () => {
        clearTimeout(timer);
        timer = setTimeout(() => this.OnListEntryChange(input), 600);
      });
    });
  ...
  async OnControlChange(input) {
    ...
    switch (type) {
      case 'bool':
        ...
        break;
      case 'color':
        newValue = this.HexToHyprColor(input.value, input.dataset.original);
        break;
      default:
    ...
    this.UpdatePreview(setting || { key, label: key, desc: '' }, newValue, sectionPath);
    ...
    applyChange(
      { root: this.state.root, rawLines: this.state.rawLines },
      sectionPath,
      key,
      newValue
    );
  ...
  AutoSetSetting(key, sectionPath, defaultVal, type) {
    ...
    this.state.root = parsed.root ?? parsed;
    this.state.rawLines = parsed.rawLines ?? this.state.rawLines;
  ...
  RenderSearchResults(results) {
    ...
    dropdown.innerHTML = results.map(r => `
      <div class="search-result" data-path='${JSON.stringify(r.path)}' data-key="${r.key}">
        ...
        <span class="search-result-value">${r.value === '[section]' ? '<em>section</em>' : this.EscapeHtml(r.value)}</span>
      </div>`).join('');
  ...
  UpdatePathDisplay() {
    const el = this.$('config-path');
    if (el && this.state.configPath) {
      const extra = this.state.fileSegments.length > 1 ? ` (+${this.state.fileSegments.length - 1} files)` : '';
      el.textContent = this.state.configPath + extra;
      el.title = this.state.fileSegments.map(s => s.filePath).join('\n');
    }
  }
  ...
  ShowNotification(msg, type = 'info') {
    ...
    clearTimeout(this.state.notifTimer);
    this.state.notifTimer = setTimeout(() => el.classList.remove('show'), 3500);
  }

  HyprColorToHex(color) {

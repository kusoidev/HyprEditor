    ...
    if (!trimmed) {
      if (comment != null) {
        return {
          type: "Comment",
          value: comment,
          loc: ConfigParser.Loc(lineNo, 0, lineNo, raw.length),
          line: lineNo,
        };
      }
      return null;
...
class ConfigParser {
  static CreateSectionNode(name = "", path = [], meta = null) {
    return {
      _type: "section",
      name,
      _path: [...path],
  ...
  static CreateValueNode(kind, key, value, line) {
    return {
      _type: kind,
      key,
      value,
  ...
  static GetValue(root, sectionPath, key) {
    let node = root;
  ...
  static GetList(root, sectionPath, key) {
    let node = root;
    for (const seg of sectionPath.map(s => String(s).toLowerCase())) {
      node = node?._children?.[seg];
  ...
  static GetDuplicateSections(root, path = []) {
    ...
    for (const [name, node] of Object.entries(root._children || {})) {
      ...
      duplicates = duplicates.concat(ConfigParser.GetDuplicateSections(node, [...path, name]));
    }

    return duplicates;
  }

  static MergeDuplicateSections(config) {
  ...
  static ApplyChange(arg1, arg2, arg3, arg4) {
  ...
  static ReplaceAssignmentValue(line, value) {
  ...
  static SerializeConfig(rawLines) {
    return rawLines.join("\n");
  }

  static GetAllSources(root) {
    return (root?._lists?.["source"] || []).map(e => e.value);
  }

  static FindAllMatches(root, query) {
    ...
    function Walk(node, path) {
      ...
      for (const [k, child] of Object.entries(node._children || {})) {
        ...
        if (child._type === "section") {
          ...
          if (label.includes(q)) {
            matches.push({ path, key: k, value: "[section]", line: child._instances[0]?.line ?? -1 });
          }
          Walk(child, [...path, k]);
        }
      }
  ...
  static RemoveDuplicateKeys(config) {
    const lines = config.rawLines.slice();
  ...
  static Loc(line, col, endLine, endCol) {
    ...
    for (let i = 0; i < line.length; i++) {
      ...
      if (ch === "#") {
        if (line[i + 1] === "#") {
          ...
          return { code: out, comment: line.slice(i + 1).trim() };
        }
      } else {
        out += ch;
      }
  ...
  static ParseValue(raw, lineNo = 0) {
    const parts = [];
    let i = 0;
    let text = "";
...
export function parseConfig(text) { return ConfigParser.ParseConfig(text); }
export function getValue(root, sectionPath, key) { return ConfigParser.GetValue(root, sectionPath, key); }
export function getList(root, sectionPath, key) { return ConfigParser.GetList(root, sectionPath, key); }
export function applyChange(arg1, arg2, arg3, arg4) { return ConfigParser.ApplyChange(arg1, arg2, arg3, arg4); }
export function serializeConfig(rawLines) { return ConfigParser.SerializeConfig(rawLines); }
export function getAllSources(root) { return ConfigParser.GetAllSources(root); }
export function findAllMatches(root, query) { return ConfigParser.FindAllMatches(root, query); }
export function getDuplicateSections(root, path) { return ConfigParser.GetDuplicateSections(root, path); }
export function mergeDuplicateSections(config) { return ConfigParser.MergeDuplicateSections(config); }
export function removeDuplicateKeys(config) { return ConfigParser.RemoveDuplicateKeys(config); }
export function normalizeConfig(config) { return ConfigParser.NormalizeConfig(config); }
